...
public class OrdersSqlProvider {
    ...
    protected void applyWhere(SQL sql, OrdersExample example, boolean includeExamplePhrase) {
        ...
        List<OrdersExample.Criteria> oredCriteria = example.getOredCriteria();
        boolean firstCriteria = true;
        for (int i = 0; i < oredCriteria.size(); i++) {
            OrdersExample.Criteria criteria = oredCriteria.get(i);
            if (criteria.isValid()) {
                if (firstCriteria) {
                    firstCriteria = false;
                } else {
                ...
                sb.append('(');
                List<OrdersExample.Criterion> criterions = criteria.getAllCriteria();
                boolean firstCriterion = true;
                for (int j = 0; j < criterions.size(); j++) {
                    OrdersExample.Criterion criterion = criterions.get(j);
                    if (firstCriterion) {
