...
import com.yfr.model.AccountExample;
...
public class AccountSqlProvider {
    ...
    protected void applyWhere(SQL sql, AccountExample example, boolean includeExamplePhrase) {
        ...
        List<AccountExample.Criteria> oredCriteria = example.getOredCriteria();
        boolean firstCriteria = true;
        for (int i = 0; i < oredCriteria.size(); i++) {
            AccountExample.Criteria criteria = oredCriteria.get(i);
            if (criteria.isValid()) {
                if (firstCriteria) {
                    firstCriteria = false;
                } else {
                ...
                sb.append('(');
                List<AccountExample.Criterion> criterions = criteria.getAllCriteria();
                boolean firstCriterion = true;
                for (int j = 0; j < criterions.size(); j++) {
                    AccountExample.Criterion criterion = criterions.get(j);
                    if (firstCriterion) {
